...
const mapPlayerForClipboard = (player: Player) => ({
  ...
  age:
    typeof player.age === "number"
      ...
      ? parseInt(player.age, 10) || undefined
      : undefined,
  ...
  const sortedPlayers = useMemo(() => {
    ...
    return [...players].sort((a, b) => {
      ...
      if (sortField === "nationality") {
        aVal =
          Array.isArray(a.citizenship) && a.citizenship[0]
            ? a.citizenship[0]
            : "";
        bVal =
          Array.isArray(b.citizenship) && b.citizenship[0]
            ? b.citizenship[0]
            : "";
      } else {
        aVal =
      ...
      <table className="player-table">
        ...
        <tbody>
                    ...
                    <div className="mobile-card-flex">
                      <div className="mobile-row-1">
                        <div className="mobile-player-name-block">
                          ...
                          <span className="mobile-player-name-text">
                            {player.name}
                          </span>
                          {player.isLbPlayer && (
                            <span className="lb-badge">LB</span>
                          )}
                        </div>
                        <div className="mobile-main-position-block">
                          <div className="mobile-main-position">
                            {player.position}
                          </div>
                          {Array.isArray(player.otherPosition) &&
                            player.otherPosition.length > 0 && (
                              <div className="mobile-other-positions">
                                {player.otherPosition.map(
                                  (pos: string, i: number) => (
                                    <div
                                      className="mobile-other-position-entry"
                                      key={i}
                                    >
                                      {pos}
                                    </div>
                      ...
                      <div className="mobile-row-2">
                        ...
                        <div className="mobile-buttons-block">
                          <span className="mobile-buttons">
                            ...
                            <button
                              className="action-button delete large-action-button"
                              onClick={() => handleDelete(player.id)}
                            >
                              Delete
                            </button>
                            <CopyToClipboard
                              player={mapPlayerForClipboard(player)}
                              className="action-button share large-action-button"
                            />
                          </span>
                        </div>
                      </div>
          ...
          {sortedPlayers.map((player: Player) => (
            <tr
              key={player.id + "-desktop"}
              className={
                player.isLbPlayer ? "lb-player desktop-only" : "desktop-only"
              }
            >
                  ...
                  player.otherPosition.length > 0 && (
                    <span className="player-other-positions">
                      {player.otherPosition?.map((pos: string, i: number) => (
                        <span key={i} className="player-other-position-entry">
                          {pos}
                        </span>
                      ))}
                    </span>
