...
const nextConfig = {
  ...
  async rewrites() {
    ...
    const backendUrl =
      process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:7771";

    if (!backendUrl) {
      console.warn(
        "NEXT_PUBLIC_API_BASE_URL is not set. API proxy rewrite disabled."
      );
      return [];
    }

    console.log(
      `Setting up API proxy rewrite from /api-proxy/* to ${backendUrl}/*`
    );
